...
function fig = plot_deconvolution_graph(Q, xx, yy, W, options)
    ...
    if options.plot_density
        fig_density = plot(xx, yy, 'm','Color', highlight,'LineWidth', options.line_width, 'DisplayName', 'f_X');
    end
    
    if options.plot_masses
        fig_masses = scatter(Q.Support, Q.ProbWeights, 'filled','MarkerFaceColor', grey, 'DisplayName', 'F_X');
    end
