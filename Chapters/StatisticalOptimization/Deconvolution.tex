...
\subsection{PhiU known}
	...
	Let $f_K$ be a bounded, even, probability density function that satisfies
	\begin{align}
...
\subsection{PhiU est from replicates}
	If the distribution of $U$ is unknown, we may still be able to estimate it if replicate measurements are present. In this scenario, we measure
	\begin{align}
		W_{jk} = X_j + U_j && j = 1, \dots, n \text{ and } k = 1, \dots, N_j
	\end{align}
	where the $X_j$ and $U_{jk}$ are independent and are identically distributed as $X$ and $U$ respectively. Following the methods of Delaigle, Hall and Meister \cite{Delaigle2008-hl}, a consistent estimator of $\phi_U$ is
	\begin{equation}
	...
	\begin{equation}
		\hat{f}_X(x) = \frac{1}{Mh} \frac{1}{2\pi} \int \euler^{-itu} \phi_K(t) / (\hat{\phi}_U(t/h) + \rho) \intd t
	\end{equation}
