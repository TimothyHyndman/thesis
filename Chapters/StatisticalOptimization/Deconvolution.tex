...
\label{sec:summary of delaigle hall}
	...
	\begin{enumerate}
		...
		\item Define
		\begin{equation}
			T(F_{\vect{\theta}, \vect{p}}) = \int_{-\infty}^\infty \left| \hat{\phi}_W(t) - \left|\hat{\psi}_W(t)\right|^{1/2} \frac{ \sum_{j = 1}^m p_j \exp(it\theta_j)}{\left| \sum_{j = 1}^m p_j \exp(it\theta_j)\right|} \right|^2 w(t) \intd t.
		\end{equation}
		...
		\end{equation}
		\item The final estimator for $F_X$ is $F_{\vect{\theta}, \hat{\vect{p}}}$.
	\end{enumerate}
