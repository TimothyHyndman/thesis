...
\subsection{PhiU known}
	...
	\end{align}
	for any fixed $h > 0$. We may estimate the density of $W$ via the usual kernel density estimator
	\begin{equation}
...
\subsection{PhiU est from replicates}
	If the distribution of $U$ is unknown, we may still be able to estimate it if replicate measurements are present. This scenario is addressed by [CITATIONS]. Here we look at the estimator of Delaigle, Hall, and Meister \cite{Delaigle2008-hl}. 
	...
	\end{equation}
	This suggests an estimator of $f_X$ given by 
	\begin{equation}
	...
	The presence of the ridge parameter is to account for potential fluctuations in $\hat{\phi}_U$ that could cause the denominator in \eqref{eq:replicates estimator} to become very small. We may alternatively use another method to prevent $\hat{\phi}_U$ from getting too close to zero such as replacing $\hat{\phi}_U(t)$ with some other function for $|t|$ large.

	A similar approach can be used in scenarios where we are able to sample directly from the distribution of $U$ [CITE].

	\subsection{U unknown}
	In the case where we do not know the distribution of $U$, nor have access to replicate measurements or direct measurements from $U$, 
