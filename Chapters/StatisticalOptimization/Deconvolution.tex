...
\label{sec:summary of delaigle hall}
	...
	\end{equation}
	where $w_1(t)$ is some non-negative weight function that assigns greater weight when $|t|$ is small. We also expect that $\hat{\phi}_U = \hat{\phi}_W / \phi_F$ should be symmetric, and have magnitude less than or equal to 1. We try to satisfy this by constructing penalties 
	\begin{align}
	...
	\begin{equation}
		T_\mathrm{min} = \min_{F \in \mathscr{F}} T(F) + \lambda_1 P1(F) + \lambda_2 P2(F),
	\end{equation}
...
[REGENERATE THESE FIGURE USING UPDATED CODE]
\begin{figure}
...
[REGENERATE THESE FIGURES USING UPDATED CODE]
\begin{figure}
	\begin{minipage}{0.5\textwidth}
