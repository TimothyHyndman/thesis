...
\label{sec:empirical results}
	...
	\subsection{The likelihood curve when \texorpdfstring{$n = 2$}{n = 2}}
		...
		\begin{proof}
			...
			Hence we need only consider the behaviour of $\vect{\gamma}(\theta;\vect{x})$ for $\theta \in [x_1,x_2]$. By the symmetry of $f$, $\hat{\vect{u}}$ must lie on the line $u_1 = u_2$\footnote{This is obvious but may need a lemma}.
		...
		\begin{lemma}
			If $f(x)$ is symmetric around $x = 0$, then $\hat{\vect{\gamma}} = (u, u)$ for some $u \in \mathbb{R}$.
		\end{lemma}
		\begin{proof}
			Let $(u, v)$ denote the coordinates of $\hat{\vect{\gamma}} \in \mathbb{R}^2$. We have
			\begin{align}
				(u, v) = p \vect{\gamma}(\theta_1; \vect{x}, f) + (1 - p) \vect{\gamma}(\theta_2;\vect{x}, f)
			\end{align}
			for some choice of $p$, $\theta_1$, and $\theta_2$.
		\end{proof}
