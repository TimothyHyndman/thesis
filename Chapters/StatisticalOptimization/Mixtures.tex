...
\label{sec:empirical results}
	...
	\label{sec:bounding Cm}
		...
		\end{theorem}
		\begin{proof}
			Let $\hat{\vect{\theta}}$ and $\hat{\vect{p}}$ represent the $m$ point maximizing mixture of $\vect{x} \in C_m$. By Lemma \ref{lem:maxkGamma}, there exists a $k^*$ such that $\Psi_{k^*}(x_i;\hat{\vect{p}},\hat{\vect{\theta}}) \geq 1$ for at least $\lceil \frac{n}{m} \rceil$ different $x_i$.
	...
	\subsection{Properties of \texorpdfstring{$\Psi$}{Psi}}
		...
		\begin{lemma}
		\label{lem:maxkGamma}
		For all $x \in \mathbb{R}$, and all discrete probability distributions with masses $\vect{p}$ at locations $\vect{\theta}$,
			\begin{equation}
