...
\section{Introduction}
	...
	In this Chapter, we will explore current results concerning $K_\vect{x}$, and expand on these to produce new bounds for $K_\vect{x}$ in a variety of settings. We look at current results in Section \ref{sec:summary of Lindsay}, and present new results in Section \ref{sec:mixture results}. Our main results are Theorems \ref{thm:n=2 inflection result}, \ref{thm:general n C1 bound}, \ref{thm:bounding Cm}, and \ref{thm:probability in C1}; as well as a simple corollary that results from this last theorem. 
	% As a corollary to this last theorem, we also obtain a simple proof of the non-consistency of $K_\vect{x}$ as an estimator of the `true' number of mass points when the data comes from a finite mixture of normals with fixed variance. 
	We also make use of an empirical analysis of $K_\vect{x}$ in Section \ref{sec:empirical results} to frame the problem from a different perspective, and to give us a point of comparison for our bounds. The discussion of current results in Section \ref{sec:summary of Lindsay} also contains some original work placed throughout where it is directly applicable. In particular, Theorem \ref{thm:all points separated by alpha} and Section \ref{sec:KKT conditions} are both new.
...
\section{Summary of Lindsay's approach}
\label{sec:summary of Lindsay}
	...
	\subsection{All points separated by \texorpdfstring{$\alpha$}{a}}
		...
		\begin{theorem}
		\label{thm:all points separated by alpha}
	...
	Theorems \ref{thm:Lindsay three statements that characterise maximizing mixture} and \ref{thm: Lindsay support is zeros of gradient} indicate that each support point of $\hat{Q}$ is a local maximum of $D_{\hat{Q}}(\vect{\theta}; \vect{x}, f)$ with respect to the parameter $\theta$. So if $f$ is parametrized by a single parameter $\theta$ and $D_Q(\theta;\vect{x},f)$ is twice differentiable in $\theta$, then each support point $\theta^*$ of the maximizing mixture distribution, $\hat{Q}$, satisfies
	\begin{align}
	...
	We will now point out that the gradient function conditions in \eqref{eq:Deq1} and \eqref{eq:Deq2} can be obtained using the Karush-Kuhn-Tucker (KKT) conditions \cite{Kuhn1951-ih} \cite{Karush2014-vk} for an associated optimisation problem. Suppose that we restrict our mixture to have no more than $m$ components. Then given a component density $f$, and data $\vect{x}$, %the problem of finding the maximum likelihood location mixture can be written as the following non-linear optimization problem.
%
	...
	Let our component density $f$ be normal with unit variance, and let $\vect{x} = (-2,2)$. Consider the mixing distribution, $\tilde{Q}_1$, that maximizes the 1 component location mixture likelihood. Let $\theta_1$ denote its single point of support. The gradient function for this distribution is given by
	\begin{equation}
...
\label{sec:empirical results}
	...
	\section{Results}
		...
		The proof of Theorem \ref{thm:n=2 inflection result} will be given in Section \ref{sec:proof of n=2 inflection result}. 
		% after some discussion concerning how the likelihood curve behaves when $n = 2$.
		%		
	...
	\subsection{Treating \texorpdfstring{$\vect{x}$}{x} as random}
		...
		\begin{equation}
			\vect{X} = (X_1, \dots, X_n),
		\end{equation}
		where the $X_i$ are independent and identically distributed as %normal with mean $\mu$ and variance $\sigma^2$, that is,
		\begin{equation}
		...
		% the likelihood ratio between $\theta = 0$ and $\theta \neq 0$ converges to $\infty$ as $n \rightarrow \infty$. 
...
One might hope to extend the results above to a wider class of densities, or to generalize the results for $n=2$ to $n > 2$. This is indeed an area of interest for future research, but there are some immediate apparent difficulties that arise if we try to use the same approaches as in this chapter. In particular, when $n=2$ we are able to simplify our problem to one concerning the curvature of the likelihood curve. For $n>2$, the problem cannot be answered by simply considering the curvature. For normal densities, we were able to obtain results for $n>2$ by taking advantage of the simple form that the derivatives of the normal density take. In general, we are not guaranteed that derivatives for other densities will simplify as well. However, we may be able to generalise our results to other densities in the one-parameter exponential family, in a similar fashion to Lindsay in \cite{Lindsay1983a-he}.
