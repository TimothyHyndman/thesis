...
\lhead{Chapter \ref{Ch:1D}. \emph{The Coupling Time on the Cycle}}
	...
	Starting with the definition of $\delta_1$, we have
		...
		\begin{align}
			\delta_1 &\leq 4n^2(2\lfloor b_n\rfloor + 1) \exp\left((z + \ln n)/\theta - \ln 2(c_n - b_n) /2 \right)\\
			&\leq 8 \exp(z/\theta) n^{3+ 1/\theta + \ln2/2 -\ln n \ln 2/ 2}
		\end{align}
	...
	\label{lem:1D prob history contained in ball}
		...
		\begin{equation}
			\prob\left[\bigcup_{u \in [0, s]}\mathcal{H}_i(t^* - u) \nsubseteq B(i, l)\right] \leq \exp\left(s - l \ln 2 \right).
		\end{equation}
	\end{lemma}
	\begin{proof}
