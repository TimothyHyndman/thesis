...
\lhead{Chapter \ref{Ch:1D}. \emph{The Coupling Time on the Cycle}}
	...
	\end{lemma}
	\begin{proof}
		From Lemma \ref{lem: U converges}, along with the fact that $0 \leq \prob(U_i^n \leq k | X_i^n = 1) \leq 1$, we have that $U_i^n | X_i^n = 1$ converges in distribution. Via the Portmanteau Theorem \cite[Theorem 25.8]{Billingsley1995-en} we immediately obtain that
		\begin{equation}
	...
	\begin{proof}
		...
		Also note that under the coupling given above, if $\alpha^\mathrm{c}$ holds, then $V_i^n \leq V_i^{n+m}$. Now we can bound the last two terms of \eqref{eq:split probs in U converge lemma}. We have
		\begin{align}
			&\prob(U_i^n \leq k, A^\mathrm{c} | X_i^n = 1) - \prob(U_i^{n+m} \leq k, \alpha^\mathrm{c} | X_i^{n+m} = 1) \\
			&= \prob\left(\left.\sum_{j \in B_i^n} \indicator(\hist_j^{n+m}(t_0) \neq \emptyset) \leq k, \alpha^\mathrm{c} \right| X_i^{n+m} = 1 \right) \notag \\ 
			&\phantom{=}- \prob(U_i^{n+m} \leq k, \alpha^\mathrm{c} | X_i^{n+m} = 1)\\
