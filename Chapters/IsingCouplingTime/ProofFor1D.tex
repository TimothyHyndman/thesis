...
\lhead{Chapter \ref{Ch:1D}. \emph{The Coupling Time on the Cycle}}
	...
	% 	t^* = (z + \ln n)/\theta.
	% \end{equation}
	% For any fixed $z \in \mathbb{R}$, $t^* > 0$ for all sufficiently large $n$. We only consider such $n$ in what follows.
	...
	Up until this point, we have chosen in our notation of several variables to omit references to their dependence on $n$. However, it will prove useful for the next section to use a superscript $n$ on some variables to make this explicit.

	\begin{lemma}
		$\lim_{n \rightarrow \infty} \lambda$ exists
	\end{lemma}

	To prove that $\lambda$ converges we will show that $U_i^n$ converges in distribution to some other random variable. This then allows us to use [WHAT??] to prove that $\lambda$ converges. The proofs require the construction of some new random variables which we define now.

	The update sequence on the cycle may be represented using the thinned processes $K_t^i$, $L_t^i$, and $R_t^i$. We may extend the update sequence into the interval $t < 0$ by mirroring these processes backwards in time from $t=0$ [HOW DO I SAY THIS PROPERLY?]. We then can use this extended update sequence to define the update support and update histories on the interval $(-\infty, t^*]$. For each $i \in V$, we define the indicators
	\begin{equation}
		\bar{X}_i^n = \begin{cases}
			1 & \forall t \in (-\infty, t^*], \mathcal{H}_i(t) \neq \emptyset,\\
			0 & \text{otherwise}.
		\end{cases}
	\end{equation}
	...
	\end{equation}
	With probability 1, $\bar{X}_i^n = 0$ and $\bar{U}_i^n = 0$ since the update histories eventually encounter an oblivious update with probability 1. Note that we use the superscript $n$ so that we can easily distinguish these random variables defined on different sized cycles.

	The following lemma relates the distribution of $U_i$ with that of $\bar{U}_i^n$.

	\begin{lemma}
		\begin{equation}
			\prob(\bar{U}_i \geq k | \bar{X}_i = 1) \leq \prob(U_i \geq k | X_i = 1) \leq \prob(\bar{U}_i \geq k | \bar{X}_i = 1) + \euler^{-z} \frac{|B_i|}{n}
		\end{equation}
	\end{lemma}
	\begin{proof}
		We start with the lower bound. We first note that the event $\{U_i \geq k\}$ depends only on the update sequence along $[0, t^*]$. So we may freely condition on the updates in $(-\infty, 0)$ (which are independent of those in $[0, t^*]$) without affecting the probability of this event. Hence
		% $\{X_i = 1\}$ coincides with the event $\{\hist_i \text{ encounters no oblivious updates along } [0, t^*]\}$. 
		% and $\bar{X}_i = 1$ have the same result along $[0, t^*]$. [WRITE THIS PROPERLY]. So
		\begin{align}
			\prob(U_i \geq k | X_i = 1) &= \prob(U_i \geq k | \hist_i \text{ doesn't die along } [0, t^*])\\
			&= \prob(U_i \geq k | \hist_i \text{ doesn't die along } (-\infty, t^*])\\
			&= \prob(U_i \geq k | \bar{X}_i = 1).
		\end{align}

		[THIS ONLY WORKS FOR 1D BECAUSE OF INDEPENDENCE STUFF... SAY MORE ABOUT THIS?]

		For every $j \in V$, if $\bar{X}_j = 1$, then $X_j = 1$. So $\bar{U}_i \leq U_i$ and
		\begin{equation}
			\prob(U_i \geq k | X_i = 1) \geq \prob(\bar{U}_i \geq k | \bar{X}_i = 1).
		\end{equation}

		We now treat the upper bound. For each $j \in B_i$, define the event
		\begin{equation}
			A_j = \{ \mathcal{H}_j(0) \neq \emptyset, \mathcal{H}_j \cap \mathcal{H}_i = \emptyset\}, 
		\end{equation}
		and define the union of these events,
		\begin{equation}
			A = \bigcup_{j \in B_i} A_j.
		\end{equation}
		We note that on the event $A^\mathrm{c} \cap \{\bar{X}_i = 1\}$, we have that $U_i = \bar{U}_i$. So 
		\begin{align}
			\prob\left(U_i \geq k | X_i = 1 \right) &= \prob\left(U_i \geq k, A | X_i = 1 \right) + \prob\left(U_i \geq k, A^\mathrm{c} | X_i = 1 \right)\\
			&= \prob[X_i = 1] ^{-1} \prob\left(U_i \geq k, A, X_i = 1\right) + \prob\left(\bar{U}_i \geq k, A^\mathrm{c}| \bar{X}_i = 1\right)\\ 
			&\leq \prob[X_i = 1] ^{-1} \prob\left(A, X_i = 1\right) + \prob\left(\bar{U}_i \geq k| \bar{X}_i = 1\right).%\\
			% &\leq \prob[X_i = 1] ^{-1} \sum_{j \in B_i} \prob[A_j, X_i = 1]
		\end{align}
%
		Now by a union bound,
		\begin{align}
			\prob[A, X_i = 1] &\leq \sum_{j \in B_i} \prob[A_j, X_i = 1]\\
				&= \sum_{j \in B_i} \prob[\hist_j(0) \neq \emptyset, \hist_i(0) \neq \emptyset, \hist_j \cap \hist_i \neq \emptyset]
		\end{align}
		and by Lemma \ref{lem:prob Xj Xi and no intersect},
		\begin{equation}
			\prob[A, X_i = 1] \leq |B_i| \prob[X_i = 1]^2.
		\end{equation}

		From \eqref{eq:1D prob X_i} we get the desired result.
	\end{proof}
