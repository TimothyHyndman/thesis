...
\lhead{Chapter \ref{Ch:1D}. \emph{The Coupling Time on the Cycle}}
	...
	Starting with the definition of $\delta_1$, we have
		...
		\begin{align}
			\delta_1 &\leq 4n^2(2\lfloor b_n\rfloor + 1) \exp\left(3(z + \ln n)/\theta - \ln 2(c_n - b_n) /2 \right)\\
			&\leq 8 \exp(3z/\theta) n^{3+ 3/\theta + \ln2/2 -\ln n / 2}
		\end{align}
	...
	\begin{lemma}
		...
		\begin{equation}
			\prob[X_j = 1, \hist_i \cap\hist_j = \emptyset|X_i = 1] \leq
		\end{equation}
	\end{lemma}
	\begin{proof}
		To begin
		\begin{align}
			\prob[X_j = 1, \hist_i \cap\hist_j = \emptyset|X_i = 1] &= n e^{z}\prob[X_j = 1, \hist_i \cap\hist_j = \emptyset, X_i = 1].
		\end{align}
		Define $S$ to be the first time that the histories intersect, or define $S = 0$ if the histories do not intersect before time $0$.
	\end{proof}