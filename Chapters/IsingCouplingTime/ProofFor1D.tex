...
\lhead{Chapter \ref{Ch:1D}. \emph{The Coupling Time on the Cycle}}
	...
	% We may extend the update sequence into the interval $t < 0$ by mirroring these processes backwards in time from $t=0$ [HOW DO I SAY THIS PROPERLY?]. We then can use this extended update sequence to define the update support and update histories on the interval $(-\infty, t^*]$. 
	...
	\label{lem: bound U by U bar}
		\begin{equation}
			\prob(\bar{U}_i^n \geq k | \bar{X}_i^n = 1) \leq \prob(U_i^n \geq k | X_i^n = 1) \leq \prob(\bar{U}_i^n \geq k | \bar{X}_i^n = 1) + \euler^{-z} \frac{|B_i|}{n}
		\end{equation}
	\end{lemma}
	\begin{proof}
		...
		\begin{align}
			\prob(U_i^n \geq k | X_i^n = 1) &= \prob(U_i^n \geq k | \hist_i^n \text{ doesn't die along } [0, t^*])\\
			&= \prob(U_i^n \geq k | \hist_i^n \text{ doesn't die along } (-\infty, t^*])\\
