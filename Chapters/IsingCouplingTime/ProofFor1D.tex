...
\lhead{Chapter \ref{Ch:1D}. \emph{The Coupling Time on the Cycle}}
	...
	Beginning with the definition of $\lambda$, we have
		...
		\end{align}
		so in order to find a lower bound for $C_\theta$ we will find an upper bound to $\expect[U_i | X_i = 1]$. Now
		\begin{align}
		...
		\begin{align}
			\lim_{n \rightarrow \infty} C_\theta \geq \frac{1}{2\sqrt{\frac{4}{\theta} - 1} - 1}.
		\end{align}
	\end{proof}
	...
	\begin{proof}
		...
		\begin{align}
			\prob[X_{i+k} = 1| X_i = 1] &\leq \prob[X_{i+k} = 1] + \prob[\mathcal{H}_i \cap \mathcal{H}_{i+k} \neq \emptyset | X_i = 1].
		\end{align}
