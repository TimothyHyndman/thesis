...
\lhead{Chapter \ref{Ch:1D}. \emph{The Coupling Time on the Cycle}}
	...
	\begin{equation}
		\bar{U}_i = \sum_{j \in B_i} \bar{X}_j.
	\end{equation}
	With probability 1, $\bar{X}_i = 0$ and $\bar{U}_i = 0$ since the update histories eventually encounter an oblivious update with probability 1. 

	The following lemma relates the distribution of $U_i$ with that of $\bar{U}_i$.

	\begin{lemma}
		\begin{equation}
			\prob(\bar{U} \geq k | \bar{X}_i = 1) \leq \prob(U_i \geq k | X_i = 1) \leq \prob(\bar{U}_i \geq k | \bar{X}_i = 1) + \euler^{-z} \frac{|B_i|}{n}
		\end{equation}
	\end{lemma}
	\begin{proof}
		We start with the lower bound. We first note that conditioning on $X_i = 1$ and $\bar{X}_i = 1$ have the same result along $[0, t^*]$. [WRITE THIS PROPERLY]. So
		\begin{equation}
			\prob(U_i \geq k | X_i = 1) = \prob(U_i \geq k | \bar{X}_i = 1).
		\end{equation}
		...
		We now treat the upper bound. Define the event
		\begin{equation}
		...
		\end{equation}
		We note that on the event $A^\mathrm{c} \cap \{\bar{X}_i = 1\}$, $U_i = \bar{U}_i$.
	\end{proof}
