...
\begin{proof}
	...
	\end{align}
	If we are conditioning on $A$, then the events $\{X_j = 1\}$ and $\{X_i = 1\}$ depend only on the update sequence within $B(j, k/2)$ and $B(i, k/2)$ resepctively. This means that
	\begin{equation}
...
\begin{proof}
	...
	\end{align}
	The events $\{\mathcal{H}_i \subset B(i, k/2)\}$ and $\{\mathcal{H}_j \subset B(j, k/2)\}$ depend only the update sequence strictly within $B(i, k/2)$ and $B(j, k/2)$ respectively. So these events are independent. Continuing
	\begin{align}
