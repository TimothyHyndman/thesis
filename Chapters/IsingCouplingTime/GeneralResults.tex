...
\begin{lemma}
	...
	\begin{proof}
		...
		For $M_w$ to occur, we require that the $l$ independent rate 1 Poisson clocks associated with each $w_k$ ring in a particular order along $[t_* - s, t_*]$. The probability of this is equal to the probability that a single rate 1 Poisson clock rings at least $l$ times in $[0, s]$, or equivalently, a rate $s$ Poisson clock rings at least $l$ times in $[0, 1]$. So we have
		\begin{equation}
			\prob[M_w] = \prob[\mathrm{Po}(s) \geq l],
		\end{equation}
		where $\mathrm{Po}(s)$ is Poisson with rate $s$. By a union bound over $\mathcal{W}$,
		\begin{align}
			\prob\left[\bigcup_{u \in [0, s]}\mathcal{H}_i(t_* - u) \nsubseteq B(i, l)\right] &\leq \prob\left[\bigcup_{w \in \mathcal{W}} M_w\right]\\
