%!TEX root = ..\..\main.tex
\chapter{General Results}
\label{Ch:GeneralResults}

\lhead{Chapter \ref{Ch:GeneralResults}. \emph{General Results}}
	...
	% stochastically dominate $(W_s, Y_s)$ by a process $(\bar{W}_s, \bar{Y}_s)$ which starts at $(\bar{W}_0, \bar{Y}_0) = (2, 0)$ and evolves in the following way. At rate $\theta \bar{W}_s$, $\bar{W}_s$ decreases by $1$, and at rate $(1 - \theta)\bar{W}_s$, both $\bar{W}_s$ and $\bar{Y}_s$ increase by $\Delta$.
	...
	% We again create a coupled process $(\bar{W}_s, \bar{Y}_s)$ such that $\bar{W}_s \geq W_s$ and $\bar{Y}_s \geq Y_s$ in the following way. We start with $(\bar{W}_s, \bar{Y}_s) = (2, 0)$ and at rate $\theta (\bar{W}_s - 1)$, $\bar{W}_s$ decreases by 1, and at rate $(1 - \theta)\bar{W}_s$, both $\bar{W}_s$ and $\bar{Y}_s$ increase by $\Delta$.

	% Let $Q_s = \exp(\alpha \bar{W}_s + \lambda\bar{Y}_s)$ where $\alpha$ and $\lambda$ are some fixed constants yet to be determined. 
	% \begin{equation}
	% 	\left.\frac{d}{ds} \expect\left[Q_{s} | Q_{s_0}\right] \right|_{s = s_0} = \left(\theta (\euler^{-\alpha} - 1)(\bar{W}_s - 1) + (1 - \theta)(\euler^{(\alpha + \lambda)\Delta} - 1)\bar{W}_s\right)Q_{s_0}
	% \end{equation}
