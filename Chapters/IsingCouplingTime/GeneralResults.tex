...
\end{lemma}
\begin{proof}
	We first relax to our alternative history construction by observing that
	\begin{align}
		\prob[\mathcal{H}_i \cap \mathcal{H}_j \neq \emptyset | X_i = 1] &\leq \prob[\hat{\mathcal{H}}_i \cap \hat{\mathcal{H}}_j \neq \emptyset | X_i = 1]
	\end{align}
	since $\mathcal{H}_i \subseteq \hat{\mathcal{H}}_i$ for any vertex $i$. Then 
	\begin{align}
		\prob[\hat{\mathcal{H}}_i \cap \hat{\mathcal{H}}_j \neq \emptyset | X_i = 1] &= \frac{\prob\left[\left\{\hat{\mathcal{H}}_i \cap \hat{\mathcal{H}}_j \neq \emptyset \right\} \cap \left\{X_i = 1\right\}\right]}{\prob[X_i = 1]}\\
		&\leq n C_z \prob\left[\left\{\hat{\mathcal{H}}_i \cap \hat{\mathcal{H}}_j \neq \emptyset \right\} \cap \left\{X_i = 1\right\}\right]\\
		&\leq n C_z \prob\left[\left\{\hat{\mathcal{H}}_i \cap \hat{\mathcal{H}}_j \neq \emptyset \right\} \cap \left(\left\{X_i = 1\right\} \cup \{X_j = 1\}\right)\right]
	\end{align}
	where $C_z = \max(\euler^{z}, \euler^{(1 - \beta \Delta)z})$. We can again use our alternative history construction by using that
	\begin{equation}
		\left\{X_i = 1\right\} = \left\{\mathcal{H}_i(0) \neq \emptyset\right\} \subseteq \left\{\hat{\mathcal{H}}_i(0) \neq \emptyset\right\},
	\end{equation}
	and so
	\begin{equation}
		\prob[\mathcal{H}_i \cap \mathcal{H}_j \neq \emptyset | X_i = 1]
		%\prob\left[\left\{\hat{\mathcal{H}}_i \cap \hat{\mathcal{H}}_j \neq \emptyset \right\} \cap \left(\left\{X_i = 1\right\} \cup \{X_j = 1\}\right)\right] 
		\leq n C_z\prob\left[\left\{\hat{\mathcal{H}}_i \cap \hat{\mathcal{H}}_j \neq \emptyset \right\} \cap \left\{\hat{\mathcal{H}}_i(0) \cup \hat{\mathcal{H}}_j(0) \neq \emptyset\right\}\right].
	\end{equation}
	Now since each update in our histories can never remove more than one vertex at a time, if $\hat{\mathcal{H}}_i(0) \cup \hat{\mathcal{H}}_j(0) = \emptyset$, then there must be some time $t > 0$ at which $\hat{\mathcal{H}}_i(t) \cup \hat{\mathcal{H}}_j(t) = \{w\}$ for a single vertex $w$. Define $S$ to be the first such time
\end{proof}