...
[MENTION ONE LEMMA WHERE WE NEED BETA SMALL IS VERY SIMILAR TO THEIR LEMMA?]
...
\end{equation}
In this chapter we consider sequences of vertex-transtitive graphs $(G_n) \in \mathscr{G}$. 

\begin{lemma}
	Define the sequence of length $L$ $d$-dimensional square lattices on a torus, $G_L = (\mathbb{Z}/\mathbb{Z} L)^d$. Then $(G_L)_{L \geq 1} \in \mathscr{G}$.
\end{lemma}
\begin{proof}
	First note that on $G_L = (\mathbb{Z}/\mathbb{Z}L)^2$, $P_n(k) \leq Q_n(k)$ and
		\begin{equation}
			Q_n(k) \leq \frac{2^d}{d!} (k+1)(k+2)\dots(k+d).
		\end{equation}
	This is a degree $d$ polynomial in $k$ which satisfies the constraints on \eqref{eq:set valid graphs}.
\end{proof}

\begin{remark}
	Another class of graphs contained in $\mathscr{G}$ are the $m$th powers of $G_L = (\mathbb{Z}/\mathbb{Z} L)^d$.	The $m$th power of a graph $G$ is a graph, $G^m$, with the same vertices as $G$, but in which all vertices whos distance in $G$ is no more than $m$ are adjacent. Clearly, $Q_n^{G^m}(k) \leq Q_n^G(mk)$. Hence powers of $G_L = (\mathbb{Z}/\mathbb{Z}L)^d$ are also in $\mathscr{G}$. This is of practical interest since the Ising model on $G^m$ corresponds to the Ising model with $m$ nearest neighbours on $G$.
\end{remark}
...
\label{sec:proof thm coupling nd}
	...
	\begin{proof}
		...
		As $n \rightarrow \infty$, the first term vanishes and we are left with \begin{align}
			\liminf_{n \rightarrow \infty} \lambda &\geq \frac{1}{1 + C_2} n m_{t^*}\\
