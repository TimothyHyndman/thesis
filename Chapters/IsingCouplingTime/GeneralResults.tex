...
\begin{proof}
	...
	\end{align}
	where we have used Corollary \ref{cor:magnetization of t star} and $C_z = \max(\euler^{z}, \euler^{(1 - \beta \Delta)z})$.
	...
	\begin{align}
		\prob[\mathcal{H}_i \cap \mathcal{H}_j \neq \emptyset | X_i = 1] &\leq n C_z \expect[\indicator_{\hat{\mathcal{H}}_i \cap \hat{\mathcal{H}}_j \neq \emptyset} m_S] \\
		&\leq n C_z \expect[\indicator_{\chi(\mathcal{H}_{ij}) \geq k-1} m_S]\\
		&\leq n C_z m_{t^*} \expect[\indicator_{\chi(\mathcal{H}_{ij}) \geq k-1} \euler^{t^* - S}]\\
		...
		&= \euler^{-k + 2} \expect[\euler^{\chi(\mathcal{H}_{ij}) + (1/2)L(\mathcal{H}_{ij}) + 1} ]
	\end{align}
