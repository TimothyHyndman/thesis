...
\section{Coupling from the Past}
	...
	\label{claim:discrete vs continuous distributions}
		Let $(N_n)_{n \in \mathbb{N}}$ be a sequence of positive random variables, and $(a_n)_{n \in \mathbb{N}}$ be a monotonic sequence of real numbers such that $N_n \geq a_n$ for all $n$ and 
		\begin{equation}
			\lim_{n \rightarrow \infty} a_n = \infty.
		\end{equation}

		Let $T(n)$ be the random time it takes for a rate $\lambda$ Poisson clock to go off $n$ times (i.e. $n$ updates in a CTMC).
	...
	\begin{proof}
		...
		\begin{equation}
			\left|\prob(N_n < \lambda w_n) - \prob\left(T(N_n) < w_n\right)\right| = \left|\prob\left(\frac{N_n}{\lambda w_n} < 1\right) - \prob\left(\frac{T(N_n)}{w_n} < 1\right)\right|.
		\end{equation}
		To do this it will be sufficient to show that the limiting distributions of $N_n/(\lambda w_n)$ and $T(N_n)/w_n$ are equal.

		Since $T(n)$ is the sum of $n$ i.i.d. exponential random variables with rate $\lambda$, 
		\begin{align}
		...
		\end{align}
		Now since $N_n \geq a_n$, for any $\epsilon_n > 0$,
		\begin{align}
			\prob\left(\left|\frac{\lambda T(N_n)}{N_n} - 1\right| \geq \epsilon_n\right) &\leq \prob\left(\left|\frac{\lambda T(a_n)}{a_n} - 1\right| \geq \epsilon_n\right).\\
			&\leq \frac{1}{a_n \epsilon_n^2}
		\end{align}
