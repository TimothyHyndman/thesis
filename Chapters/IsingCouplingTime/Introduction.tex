...
\section{Coupling from the Past}
	...
	\begin{proof}
		...
		\end{align}
		So for any $\epsilon > 0, C > 1$
		\begin{equation}
			\prob(|Y_n - Z_n| > \epsilon) \leq \prob\left(\left|\frac{T(N_n)\lambda}{N_n} - 1\right| > \epsilon\frac{a_n}{C b_n} \right) + \prob\left(\frac{N_n}{\lambda a_n} > \frac{C b_n}{a_n} \right).
		\end{equation}
		We will show that both of the terms on the right hand side vanish as $n \rightarrow \infty$. We start with the first of these.

		Since $N_n \geq m_n$, 
		\begin{align}
			\prob\left(\left|\frac{T(N_n)\lambda}{N_n} - 1\right| > \epsilon\frac{a_n}{C b_n} \right) &\leq \prob\left(\sup_{k \geq m_n}\left|\frac{T(k)\lambda}{k} - 1\right| > \epsilon\frac{a_n}{C b_n} \right)\\
			&= \prob\left(\bigcup_{k \geq m_n} \left\{ \left|\frac{T(k)\lambda}{k} - 1\right| > \epsilon\frac{a_n}{C b_n} \right\}\right)\\
			&\leq \sum_{k = m_n}^\infty \prob\left(\left|\frac{T(k)\lambda}{k} - 1\right| > \epsilon\frac{a_n}{C b_n} \right).%\\
			% &\leq \sum_{k = m_n}^\infty
		\end{align}

		Note that $T(k)$ is the sum of $k$ i.i.d. exponential random variables with rate $\lambda$. blahblahblah...

		Now
		\begin{align}
			\prob\left(\frac{N_n}{\lambda a_n} > \frac{C b_n}{a_n} \right) &= \prob\left(Z_n > (C - 1)\frac{b_n}{a_n} \right)
		\end{align}
		and since $b_n / a_n \rightarrow \infty$, $C > 1$, and $Z_n$ converges
		\begin{equation}
			\lim_{n\rightarrow \infty} \prob\left(\frac{N_n}{\lambda a_n} > \frac{C b_n}{a_n} \right) = 0.
		\end{equation}
