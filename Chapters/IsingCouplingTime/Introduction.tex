...
\section{Coupling from the Past}
	...
	\begin{claim}
		Let $T_n$ be a sequence of random times that converges to $\infty$ in some sense. Let $N(T_n)$ be the number of updates that occur in the rate $\lambda$ CTMC in time $T_n$. Let $w_n$ be some sequence such that
		\begin{equation}
			\lim_{n \rightarrow \infty} \prob(T_n < w_n) = L
		\end{equation}
		for some finite $L$.
		Then
		\begin{equation}
			\lim_{n \rightarrow \infty} \prob(N(T_n) < \lambda w_n) = L.
		\end{equation}
	\end{claim}
	\begin{proof}
		\begin{align}
			\prob(T_n < w_n) &= \prob\left(1 < \frac{w_n}{T_n}\right)\\
			&= \prob\left(N(T_n) < \frac{N(T_n)}{T_n} w_n\right)
		\end{align}

		Now for a CTMC with updates at constant rate $\lambda$, 
		\begin{equation}
		...
		So 
		\begin{align}
			\lim_{n \rightarrow \infty} \prob(T_n < w_n) &= \lim_{n \rightarrow \infty} \prob\left(N(T_n) < \frac{N(T_n)}{T_n} w_n\right)\\
			&= \lim_{n \rightarrow \infty} \prob(N(T_n) < \lambda w_n)
		\end{align}
