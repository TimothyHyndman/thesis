...
\section{Compound Poisson Approximation}
	...
	\begin{equation}
		t^* = a_L z + d_L.
	\end{equation}
	The specific choices for $a_L$ and $d_L$ depend on the graph $G_L$. We choose these such that for large enough $L$, $t^* > 0$, and then only consider these $L$. For each $i \in V$, define the indicator
	\begin{equation}
